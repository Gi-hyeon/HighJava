...
/**
 * Service객체는 DAO 만들어진 메서드를 원하는 작업에 맞게 호출하여
 * 결과를 받아오고, 받아온 결과를 Controller에게 보내주는 역할을 한다.
 * 
 * (자바고급 과정에서는 보통 DAO와 메서드의 구조가 같게한다.)
 * 
 * @author PC-08
 *
 */
public interface IMemberService {
	 ...
	 * @param memVo inser할 데이터가 저장된 MemberVO객체
	...
	/**
	 * DB의 전체 회원 정보를 겨자와서 List에 담아서 반환하는 메서드
	 * 
	...
	/**
	 * 매개변수로 받은 Map 을 이용하여 회원 정보 중 원하는 컬럼을 수정하는 메서드 -> 수정 항목이 1개
	...
	/**
	 * 매개변수로 원하는 Map을 이용하여 원하는 항목을 수정하는 메서드 - 수정항목이 여러개...
	 * Map의 정보 -> key값 : 수정할 컬럼명(검색할 ID는 'memId'), value값 : 수정할 데이터 값(검색할 ID값 포함)
	 * 
	 * @param dataMap 수정할 정보가 저장된 객체
	 * @return 작업 성공 : 1, 작업 실패 : 0 
	 */
